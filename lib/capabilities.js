...

  {
    ...
    async check() {
      ...
      const hasTool = tools.some(t =>
        t.name === 'web_search' || t.name === 'search_web' || t.name === 'google_search'
      );
      if (hasTool) return { ready: true };

      // Check for known search API credentials
      const serper = await getCredential('serper_api_key');
      const brave = await getCredential('brave_search_key');

      if (serper || brave) {
        return {
          ready: false,
          missing: 'web_search custom tool (API key is configured)',
          setup: serper
            ? 'Build a web_search tool using Serper API (serper.dev): POST to https://google.serper.dev/search with header X-API-KEY from get_credential("serper_api_key"), body {q: query}. Return the organic results.'
            : 'Build a web_search tool using Brave Search API: GET https://api.search.brave.com/res/v1/web/search?q={query} with header X-Subscription-Token from get_credential("brave_search_key"). Return results.',
        };
      }

      return {
        ready: false,
        missing: 'Search API key + web_search tool',
        setup: 'IMPORTANT: You CANNOT scrape Google directly with fetch() — it returns useless HTML/JS. You need a search API. Options: 1) Serper (serper.dev) — free tier, 2500 searches/mo, store key as serper_api_key. 2) Brave Search API (brave.com/search/api) — free tier, 2000/mo, store key as brave_search_key. Ask the user which they prefer and guide them to get a key. Then build a web_search tool that calls the API.',
      };
    },
  },
