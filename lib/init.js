...
async function tuiSelect(title, options, opts = {}) {
  ...
  function draw() {
    ...
    drawBox(startRow, startCol, maxW, boxH, 'BetterClaw Setup');
...
async function tuiInput(title, prompt, defaultVal, opts = {}) {
  ...
  function draw() {
    ...
    drawBox(startRow, startCol, maxW, 8, 'BetterClaw Setup');
    drawText(startRow + 1, startCol + 3, `${c.bold}${title}${c.reset}`);
...
async function tuiConfirm(title, message, defaultYes = true) {
  ...
  function draw() {
    ...
    drawBox(startRow, startCol, maxW, 7, 'BetterClaw Setup');
    drawText(startRow + 1, startCol + 3, `${c.bold}${title}${c.reset}`);
...
async function runWizard() {
  ...
  // ─── Welcome ───
  const action = await tuiSelect('Welcome to BetterClaw', [
  ...
  if (doLink) {
    ...
    try {
      execSync('npm link', { cwd: resolve(import.meta.dirname, '..'), stdio: 'inherit' });
    } catch {
  ...
  drawText(centerR + 2, centerC + 4, `${c.green}✓${c.reset} BetterClaw is ready`);
