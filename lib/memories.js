...
import { graphRecall } from './graph-memory.js';
...
export async function recall(topic) {
  ...
  // Graph search first
  try {
    const graphResults = await graphRecall(topic);
    if (graphResults) results.push(graphResults);
  } catch { /* graph unavailable — silent */ }

  // Flat file fallback
  try {
    const content = await readFile(topicPath(topic), 'utf-8');
