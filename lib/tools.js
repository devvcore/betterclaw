...
The "prompt" is what the disposable agent receives as its task. Write it like instructions: "Check my email inbox and summarize unread messages in the journal."`,
  ...
  {
    name: 'run_command',
    description: `Run a shell command on the user's machine and return stdout + stderr. Use this for anything that needs the local system: installing packages, playing media, opening apps, running scripts, git, brew, yt-dlp, ffmpeg, etc.

Commands run in a shell (/bin/sh) with a timeout. Long-running or interactive commands are not supported — use for quick, fire-and-forget operations.

Examples:
- run_command({ command: "open https://example.com" }) — open a URL in the browser
- run_command({ command: "afplay ~/Music/song.mp3 &" }) — play audio in background
- run_command({ command: "brew list" }) — list installed packages
- run_command({ command: "yt-dlp -x --audio-format mp3 -o '/tmp/song.mp3' 'https://youtube.com/...' && open /tmp/song.mp3" }) — download and play`,
    parameters: {
      type: 'object',
      properties: {
