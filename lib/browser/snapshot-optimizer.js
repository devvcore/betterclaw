 ...
 *
 * Designed to work on OpenClaw ref-annotated snapshots ([ref=eXX] tags).
...
// ── 1. OpenClaw-equivalent baseline (add refs to raw snapshot) ──────────
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (p.role === 'listitem' || p.role === 'row' || p.role === 'article') {
      ...
      if (tracker.count <= maxItems) {
        ...
        const droppedLines = lines.slice(i, lines.length);
...
// When a parent (link/button) has a long name that already contains the text
// of its children (heading, paragraph, text), remove the children.
// This is the #1 waste source on news sites like BBC, Reddit, etc.
...
// Remove unnamed text nodes, /placeholder annotations, empty lines, etc.
